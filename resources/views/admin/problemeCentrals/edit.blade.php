...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.problemeCentral.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.probleme-centrals.update", [$problemeCentral->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="code_probleme_central">{{ trans('cruds.problemeCentral.fields.code_probleme_central') }}</label>
                <input class="form-control {{ $errors->has('code_probleme_central') ? 'is-invalid' : '' }}" type="text" name="code_probleme_central" id="code_probleme_central" value="{{ old('code_probleme_central', $problemeCentral->code_probleme_central) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $problemeCentral->description) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('resultat_intermediaire') ? 'is-invalid' : '' }}" name="resultat_intermediaire_id" id="resultat_intermediaire_id" required>
                    @foreach($resultat_intermediaires as $id => $resultat_intermediaire)
                        <option value="{{ $id }}" {{ ($problemeCentral->resultat_intermediaire ? $problemeCentral->resultat_intermediaire->id : old('resultat_intermediaire_id')) == $id ? 'selected' : '' }}>{{ $resultat_intermediaire }}</option>
                    @endforeach
                </select>
