...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.reponse.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.reponses.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('questionnaire') ? 'is-invalid' : '' }}" name="questionnaire_id" id="questionnaire_id" required>
                    @foreach($questionnaires as $id => $questionnaire)
                        <option value="{{ $id }}" {{ old('questionnaire_id') == $id ? 'selected' : '' }}>{{ $questionnaire }}</option>
                    @endforeach
                </select>
