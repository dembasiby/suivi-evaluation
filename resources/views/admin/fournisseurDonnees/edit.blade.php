...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.fournisseurDonnee.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.fournisseur-donnees.update", [$fournisseurDonnee->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ ($fournisseurDonnee->user ? $fournisseurDonnee->user->id : old('user_id')) == $id ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('organisation') ? 'is-invalid' : '' }}" name="organisation_id" id="organisation_id" required>
                    @foreach($organisations as $id => $organisation)
                        <option value="{{ $id }}" {{ ($fournisseurDonnee->organisation ? $fournisseurDonnee->organisation->id : old('organisation_id')) == $id ? 'selected' : '' }}>{{ $organisation }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('point_focal') ? 'is-invalid' : '' }}" name="point_focal_id" id="point_focal_id" required>
                    @foreach($point_focals as $id => $point_focal)
                        <option value="{{ $id }}" {{ ($fournisseurDonnee->point_focal ? $fournisseurDonnee->point_focal->id : old('point_focal_id')) == $id ? 'selected' : '' }}>{{ $point_focal }}</option>
                    @endforeach
                </select>
