...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.extrant.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.extrants.update", [$extrant->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="code_extrant">{{ trans('cruds.extrant.fields.code_extrant') }}</label>
                <input class="form-control {{ $errors->has('code_extrant') ? 'is-invalid' : '' }}" type="text" name="code_extrant" id="code_extrant" value="{{ old('code_extrant', $extrant->code_extrant) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $extrant->description) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('effet_immediat') ? 'is-invalid' : '' }}" name="effet_immediat_id" id="effet_immediat_id" required>
                    @foreach($effet_immediats as $id => $effet_immediat)
                        <option value="{{ $id }}" {{ ($extrant->effet_immediat ? $extrant->effet_immediat->id : old('effet_immediat_id')) == $id ? 'selected' : '' }}>{{ $effet_immediat }}</option>
                    @endforeach
                </select>
