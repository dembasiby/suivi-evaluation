...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.effetImmediat.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.effet-immediats.update", [$effetImmediat->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="code_effet_immediat">{{ trans('cruds.effetImmediat.fields.code_effet_immediat') }}</label>
                <input class="form-control {{ $errors->has('code_effet_immediat') ? 'is-invalid' : '' }}" type="text" name="code_effet_immediat" id="code_effet_immediat" value="{{ old('code_effet_immediat', $effetImmediat->code_effet_immediat) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $effetImmediat->description) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('effet_intermediaire') ? 'is-invalid' : '' }}" name="effet_intermediaire_id" id="effet_intermediaire_id" required>
                    @foreach($effet_intermediaires as $id => $effet_intermediaire)
                        <option value="{{ $id }}" {{ ($effetImmediat->effet_intermediaire ? $effetImmediat->effet_intermediaire->id : old('effet_intermediaire_id')) == $id ? 'selected' : '' }}>{{ $effet_intermediaire }}</option>
                    @endforeach
                </select>
